...
namespace LightCreator
{
    ...
    public class PointLightComponent : GH_Component
    {
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            pManager.AddBooleanParameter("run", "run", "Button | 开关", GH_ParamAccess.item);
            pManager.AddPointParameter("location", "location", "Location of PointLight | 球形灯基点位置", GH_ParamAccess.list);
            pManager.AddNumberParameter("intensity", "intensity", "Light Intensity | 灯光亮度", GH_ParamAccess.item, 100);
